...
export const loginService = async (payload: LoginDTO) => {
  ...
  return {
    requires2FA: false,
    user: { id: user.id, correo: user.correo, nombre: user.nombre, apellido: user.apellido, rol: user.rol },
    token
  }
}
...
export const verify2FAService = async ({ userId, codigo }: Verify2FADTO) => {
  ...
  if (!userId || !normalizedCode) throw new AuthError('El usuario y el código son obligatorios', 400)
  if (!/^\d{6}$/.test(normalizedCode)) throw new AuthError('El código debe tener exactamente 6 dígitos numéricos', 400)

  const user = await findUserById(userId)
  if (!user) throw new AuthError('Usuario no encontrado', 404)
  if (!user.two_factor_activo) throw new AuthError('El usuario no tiene autenticación en dos pasos activada', 400)
...
export const registerUser = async (payload: RegisterDTO) => {
  ...
  const verificationToken = generatePendingRegisterToken({
    ...
    nonce,
    codeSignature: signRegisterCode({ codigo, correo: normalized.correo, nonce })
  })
...
export const verifyRegisterCodeService = async (payload: VerifyRegisterCodeDTO) => {
  ...
  const expectedSignature = signRegisterCode({ codigo, correo: decoded.correo, nonce: decoded.nonce })
...
export const getMeService = async (token: string) => {
  ...
  if (session.usuario.activo === false) throw new AuthError('Esta cuenta está desactivada', 403)
...
export const activate2FAService = async ({ userId, password }: VerifyPasswordDTO) => {
  ...
  return { message: 'Verificación en dos pasos activada correctamente', two_factor_activo: true }
}
...
export const deactivate2FAService = async (userId: number) => {
  ...
  return { message: 'Verificación en dos pasos desactivada correctamente', two_factor_activo: false }
}
...
export const loginWithGoogleCodeService = async (code: string) => {
  ...
  if (!userInfoResponse.ok) throw new Error('No se pudo obtener la información de la cuenta de Google')

  const correo = googleUser.email?.trim().toLowerCase()
  if (!correo || googleUser.verified_email === false) throw new Error('Google no devolvió un correo válido')

  const user = await findUserByCorreo(correo)
  if (!user) throw new AuthError('Esta cuenta de Google no está registrada. Regístrate primero.', 404)
  if (user.activo === false) throw new AuthError('Esta cuenta está desactivada', 403)
...
export const forgotPasswordService = async (payload: ForgotPasswordDTO) => {
  ...
  await createPasswordRecovery({ usuarioId: user.id, token: resetToken, expiraEn })
  ...
  if (!emailResult.success) throw new Error('No se pudo enviar el correo de recuperación. Intenta nuevamente.')
...
export const resetPasswordService = async (payload: ResetPasswordDTO) => {
  ...
  if (!token || !password || !confirmPassword) throw new AuthError('Todos los campos son obligatorios', 400)
  if (password !== confirmPassword) throw new AuthError('Las contraseñas no coinciden', 400)
  if (password.length < 8) throw new AuthError('La contraseña debe tener al menos 8 caracteres', 400)
  if (!/[A-Z]/.test(password)) throw new AuthError('La contraseña debe contener al menos una mayúscula', 400)
  if (!/[0-9]/.test(password)) throw new AuthError('La contraseña debe contener al menos un número', 400)
  if (!/[^A-Za-z0-9]/.test(password)) throw new AuthError('La contraseña debe contener al menos un carácter especial', 400)

  const recovery = await findPasswordRecoveryByToken(token)
  if (!recovery || !recovery.activo) throw new AuthError('El enlace no es válido o ya fue utilizado', 400)
  if (new Date() > recovery.expiraEn) throw new AuthError('El enlace ha expirado. Solicita uno nuevo.', 400)
