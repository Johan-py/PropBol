...
export const getInmueblesRecomendados = async (req: Request, res: Response) => {
  try {
    ...
    const maxPrice = req.query.maxPrice
      ...
      : undefined;
    const minSuperficie = req.query.minSuperficie
      ? Number(req.query.minSuperficie)
      : undefined;
    const maxSuperficie = req.query.maxSuperficie
      ? Number(req.query.maxSuperficie)
      : undefined;
    const dormitoriosMin = req.query.dormitoriosMin
      ? Number(req.query.dormitoriosMin)
      : undefined;
    const dormitoriosMax = req.query.dormitoriosMax
      ? Number(req.query.dormitoriosMax)
      : undefined;
    const banosMin = req.query.banosMin
      ? Number(req.query.banosMin)
      : undefined;
    const banosMax = req.query.banosMax
      ? Number(req.query.banosMax)
      : undefined;
...
export const ordenarPorAfinidad = async (req: Request, res: Response) => {
  try {
    ...
    if (!Array.isArray(inmuebleIds) || inmuebleIds.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: "inmuebleIds es requerido" });
    }

    const resultado = await recomendacionesService.ordenarPorAfinidad(
      inmuebleIds,
      usuarioId,
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Error al ordenar por afinidad" });
  }
};
