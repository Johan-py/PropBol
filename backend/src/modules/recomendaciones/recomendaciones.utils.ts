...
export class ScoreCalculator {
  ...
  calcularScoreAvanzado(
    ...
    score: number;
    razones: string[];
    similaridadFuerte: SimilaridadFuerte;
  } {
    let scoreTotal = 0;
    const razones: string[] = [];
    ...
    for (const favorito of favoritos) {
      ...
      if (
        precioFav > 0 &&
        Math.abs(precioActual - precioFav) / precioFav <= 0.2
      ) {
        puntosFav += 4;
      ...
      if (
        supFav > 0 &&
        supActual > 0 &&
        Math.abs(supActual - supFav) / supFav <= 0.2
      ) {
        puntosFav += 3;
    ...
    if (esAvanzado) {
      ...
      if (coincidencias >= 3) {
        ...
        razones.push(
          `Similaridad fuerte (${coincidencias}/4) +${config.pesoSimilaridadFuerte}pts`,
        );
      }
