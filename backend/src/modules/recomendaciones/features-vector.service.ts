...
export class FeaturesVectorService {
  ...
  async obtenerContextoUsuario(usuarioId: number): Promise<ContextoUsuario> {
    ...
    for (const { inmueble, peso } of todos) {
      ...
      for (const a of inmueble.inmueble_amenidad) {
        amenidadCount.set(
          a.amenidad_id,
          (amenidadCount.get(a.amenidad_id) || 0) + peso,
        );
      }
    }
  ...
  async extraerVector(
    ...
    const f_categoriaMatch = contexto.categoriasPreferidas.includes(
      inmueble.categoria || "",
    )
      ? 1
      : 0;
    const f_tipoAccionMatch =
      contexto.tipoAccionPreferida === inmueble.tipoAccion ? 1 : 0;
    ...
    const f_similitudPrecio =
      contexto.precioPromedio > 0
        ? Math.max(
            0,
            1 -
              Math.abs(precio - contexto.precioPromedio) /
                contexto.precioPromedio,
          )
        : 0;

    // Amenidades en común con las preferidas del usuario
    const amenidadesInmueble = new Set(
      inmueble.inmueble_amenidad.map((a) => a.amenidad_id),
    );
    ...
    const f_precioReducido =
      inmueble.precio_anterior && Number(inmueble.precio_anterior) > precio
        ? 1
        : 0;
   ...
   */
  async extraerVectores(
    inmuebleIds: number[],
    usuarioId: number,
  ): Promise<FeatureVector[]> {
    const contexto = await this.obtenerContextoUsuario(usuarioId);
    return Promise.all(
      inmuebleIds.map((id) => this.extraerVector(id, contexto)),
    );
  }
}
