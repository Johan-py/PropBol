...
export class FeaturesService {
  ...
  async recomendar(
    ...
    try {
      ...
      for (const v of historial) {
        const horasDiff =
          (ahora.getTime() - new Date(v.vistaEn).getTime()) / (1000 * 3600);
        const pesoRecencia =
          horasDiff < 1 ? 8 : horasDiff < 24 ? 4 : horasDiff < 168 ? 2 : 1;
        interacciones.set(
          v.inmuebleId,
          (interacciones.get(v.inmuebleId) || 0) + pesoRecencia,
        );
      }
      for (const f of favoritos) {
        interacciones.set(
          f.inmuebleId,
          (interacciones.get(f.inmuebleId) || 0) + 5,
        ); // favoritos suman más
      }
      console.log("[ML] Interacciones totales:", interacciones.size);
      console.log(
        "[ML] IDs en interacciones:",
        Array.from(interacciones.keys()),
      );
      ...
      for (const idInteractuado of inmueblesInteractuados) {
        const idxInteractuado = propiedadesSimilares.findIndex(
          (p) => p.id === idInteractuado,
        );
        ...
        for (let i = 0; i < propiedadesSimilares.length; i++) {
          ...
          scores.set(
            propiedadesSimilares[i].id,
            scoreActual + similitud * peso,
          );
          ...
          if (
            similitud > 0.3 &&
            !razones.includes(
              `Similar a "${this.obtenerTitulo(idInteractuado)}"`,
            )
          ) {
            razones.push(
  ...
  private async obtenerInmueblesCandidatos(
    ...
    const whereCategoria =
      categorias.length > 0 ? { in: categorias } : undefined; // ← AGREGAR ESTO
    ...
    if (
      filtrosActivos?.modoInmueble &&
      filtrosActivos.modoInmueble.length > 0
    ) {
  ...
  private extraerCaracteristicas(inmuebles: any[]): number[][] {
    ...
    const maxSuperficie = Math.max(
      ...inmuebles.map((p) => Number(p.superficieM2) || 0),
    );

    return inmuebles.map((inm) => {
      const precioNorm = Number(inm.precio) / (maxPrice || 1);
      const superficieNorm =
        (Number(inm.superficieM2) || 0) / (maxSuperficie || 1);
