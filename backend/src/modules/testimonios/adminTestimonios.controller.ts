...
export const createAdminTestimonio = async (req: Request, res: Response) => {
  try {
    const { comentario, ciudad, zona, categoria, visible, nombreAutor, apellidoAutor, calificacion } = req.body as {
      comentario?: string
      ciudad?: string
      zona?: string
      categoria?: string
...
export const updateAdminTestimonio = async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      id: actualizado.id,
      nombreTestimonial: `${finalUser?.nombre || ''} ${finalUser?.apellido || ''}`.trim() || 'Usuario',
      creadoPor: 'Admin',
