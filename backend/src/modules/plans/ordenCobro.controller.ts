...
export const crearOrdenCobro = async (req: Request, res: Response) => {
  try {
    ...
    if (!planId || !tipo) return res.status(400).json({ error: 'Faltan datos' })

    const plan = await prisma.plan_suscripcion.findUnique({ where: { id: planId } })
    ...
    if (montoNeto < 0) return res.status(400).json({ error: 'Error de cálculo, total no válido' })

    // Usar la tabla suscripciones_activas (ya existe)
    ...
    // Crear transacción (orden de cobro)
...
export const cambiarPlan = async (req: Request, res: Response) => {
  try {
    ...
    const plan = await prisma.plan_suscripcion.findUnique({ where: { id: nuevoPlanId } })
    if (!plan) return res.status(404).json({ error: 'Plan no encontrado' })

    const nuevoTotal = plan.precio_plan?.toNumber() || 0
    if (nuevoTotal < 0) return res.status(400).json({ error: 'Error de cálculo, total no válido' })
