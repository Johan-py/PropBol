...
export const trackSearch = async (req: Request, res: Response) => {
  try {
    ...
    const result = await telemetriaService.trackSearch(
      usuarioId,
      ip,
      searchData,
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Error al guardar telemetría" });
  }
};
...
export const getRecomendados = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error al obtener recomendados" });
  }
};
