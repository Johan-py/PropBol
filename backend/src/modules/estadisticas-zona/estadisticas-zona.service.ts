...
export const estadisticasZonaService = {
  async getEstadisticas(
    params: EstadisticasZonaParams,
  ): Promise<EstadisticasZonaResult> {
    ...
    const precios: number[] = inmuebles.map((i: InmuebleStats): number =>
      Number(i.precio),
    );
    const promedioPrecio: number =
      precios.reduce((a: number, b: number): number => a + b, 0) /
      precios.length;
    ...
    for (const inmueble of inmueblesRecientes) {
      const fecha: Date | null =
        inmueble.fechaPublicacion ?? inmueble.createdAt;
      if (!fecha) continue;
    ...
    const evolucionPrecios: EvolucionMes[] = mesesOrdenados.map(
      (clave: string): EvolucionMes => {
        const [año, mes] = clave.split("-");
        const preiosMes: number[] = porMes[clave] ?? [];
        const promedio: number =
          preiosMes.reduce((a: number, b: number): number => a + b, 0) /
          preiosMes.length;
        return {
    ...
    const distribucionPorCategoria: DistribucionCategoria[] = Object.entries(
      porCategoria,
    ).map(
