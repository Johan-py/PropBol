...
const TIPOS_ACCION_VALIDOS: TipoOperacion[] = [
  "VENTA",
  "ALQUILER",
  "ANTICRETO",
];
...
export class EstadisticasZonaController {
    ...
    try {
      ...
      if (!zonaId || !tipoOperacion) {
        return res.status(400).json({
          ok: false,
          mensaje:
            "Debes proporcionar una zona y un tipo de operación para consultar las estadísticas.",
        });
      }
      ...
      if (error instanceof Error) {
        ...
        if (error.message === "SIN_DATOS_SUFICIENTES") {
          return res.status(200).json({
            ...
            sinDatos: true,
            mensaje:
              "No hay datos suficientes para esta zona y tipo de operación.",
          });
        }
  ...
  static async getZonas(req: Request, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, mensaje: "Error al obtener las zonas." });
    }
  }
}
