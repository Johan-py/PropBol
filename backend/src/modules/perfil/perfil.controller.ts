...
export const listarMisPublicaciones = async (req: AuthRequest, res: Response) => {
  try {
    const usuarioId = req.usuario?.id
    ...
    const publicaciones = await publicacionesService.listarMisPublicaciones(usuarioId)

    // Obtener estadísticas de publicaciones y suscripción
    const estadisticas = await publicacionesService.obtenerEstadisticasPublicaciones(usuarioId)
...
export const obtenerPreferenciasNotificacion = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.json({
      ...
      tieneTelefono:
        usuario.telefonos.length > 0 || usuario.telefono_telefono_usuario_idTousuario.length > 0
    })
  } catch (error) {
