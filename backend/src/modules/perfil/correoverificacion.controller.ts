...
export const cambiarPassword = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (usuario.bloqueo_cambio_password_hasta && ahora >= usuario.bloqueo_cambio_password_hasta) {
    ...
    if (passwordIncorrecta) {
      const nuevosIntentos = Math.min(usuario.intentos_fallidos_cambio_password + 1, 5)
