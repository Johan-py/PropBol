...
const mapNotificationToFrontend = (notification: {
  ...
  return {
    ...
    status: notification.leida ? 'leida' : ('no leida' as const),
    archived: notification.archivada
  }
}
...
export const getNotificationsService = async (
  ...
  const [notifications, total] = await Promise.all([
    findNotificationsByUserRepository({ usuarioId, filter, limit, offset }),
    countNotificationsByUserRepository({ usuarioId, filter })
  ])
...
export const markNotificationAsReadService = async (id: number, usuarioId: number) => {
  ...
  const notification = await findNotificationByIdRepository({ id, usuarioId })
...
export const deleteNotificationService = async (id: number, usuarioId: number) => {
  ...
  await softDeleteNotificationRepository({ id, usuarioId })
...
export const archiveNotificationService = async (id: number, usuarioId: number) => {
  ...
  if (notification.archivada) {
    throw new ServiceError('La notificación ya está archivada', 400)
  }

  await archiveNotificationRepository({
    id,
    usuarioId,
    fechaLectura: new Date()
  })

  return { message: 'Notificación archivada correctamente' }
}
