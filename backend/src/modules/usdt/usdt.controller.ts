...
import {
  convertirBobAUsdt,
  getExchangeRate,
  verificarTransaccionShasta,
} from "./usdt.service.js";
...
export const crearPagoUsdt = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!transaccionId)
      return res.status(400).json({ error: "Falta transaccionId" });
    ...
    if (transaccion.id_usuario !== usuarioId)
      return res.status(403).json({ error: "Sin permiso" });
    if (transaccion.estado !== "PENDIENTE") {
      return res
        .status(409)
        .json({ error: "La transacción ya no está pendiente" });
    }
...
export const verificarPagoUsdt = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!txHash?.trim())
      return res
        .status(400)
        .json({ error: "Falta el hash de la transacción (TX ID)" });
    ...
    if (transaccion.metodo_pago !== "USDT_TRC20") {
      return res
        .status(400)
        .json({ error: "Esta transacción no corresponde a un pago USDT" });
    }
    ...
    if (transaccion.estado === "CANCELADO") {
      return res
        .status(400)
        .json({ error: "Esta orden fue cancelada. Genera una nueva." });
    }
    ...
    if (Date.now() > fechaExpiracion.getTime()) {
      ...
      return res
        .status(400)
        .json({ error: "Tiempo de pago expirado. Genera una nueva orden." });
    }
    ...
    await prisma.bitacora_pagos.create({
      data: {
        ...
        id_transaccion: id,
        evento: resultado.valida
          ? "USDT_VERIFICADO_OK"
          : "USDT_VERIFICADO_FALLO",
...
export const obtenerTipoCambio = (_req: Request, res: Response) => {
  ...
  const walletAddress = process.env.TRON_WALLET_ADDRESS ?? "";
  const red =
    process.env.TRON_NETWORK === "shasta" ? "Shasta Testnet" : "Mainnet";
