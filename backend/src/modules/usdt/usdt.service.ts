  ...
  try {
    ...
    if (!res.ok) {
      return {
        valida: false,
        confirmaciones: 0,
        mensaje: "Transacción no encontrada en la red",
      };
    }

    const data = (await res.json()) as {
      data?: { ret?: { contractRet?: string }[]; blockNumber?: number }[];
    };
    ...
    if (!tx) {
      return {
        valida: false,
        confirmaciones: 0,
        mensaje: "Hash de transacción no encontrado",
      };
    }
    ...
    if (!exitosa) {
      return {
        valida: false,
        confirmaciones: 0,
        mensaje: "Transacción no exitosa o pendiente de confirmación",
      };
    }
    ...
    return {
      valida: false,
      confirmaciones: 0,
      mensaje: "Error al conectar con la red Tron",
    };
  }
};
