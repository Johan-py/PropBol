...
export const crearPublicacion = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ estado: "Pendiente de revisión", message: "NOT_AUTHENTICATED" });
    }
    ...
    return res.status(201).json({
      estado: "Validado",
      mensaje: "Publicación lista para confirmar",
      data: nueva,
    });
  } catch (error: unknown) {
    if (error instanceof Error) {
      ...
      return res.status(400).json({ estado: "Pendiente de revisión", message: error.message });
    }
...
export const flujoPublicacion = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res.status(200).json({ message: "FLOW_ALLOWED" });
  } catch (error: unknown) {
    if (error instanceof Error) {
      if (error.message === "LIMIT_REACHED") {
        return res.status(403).json({
