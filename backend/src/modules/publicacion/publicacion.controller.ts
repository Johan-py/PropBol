...
export const confirmarPublicacionController = async (req: AuthRequest, res: Response) => {
  ...
  try {
    const resultado = await confirmarPublicacionService(publicacionId, Number(usuarioSolicitanteId))
...
export const editarMultimediaPublicacionController = async (req: AuthRequest, res: Response) => {
  const publicacionId = Number(req.params.id)
  const usuarioSolicitanteId = req.user?.id
