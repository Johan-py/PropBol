...
const esVideoPermitido = (url: string) => {
  ...
  return valor.includes('youtube.com') || valor.includes('youtu.be') || valor.includes('vimeo.com')
}
...
const parseImagenesAEliminar = (valor: unknown): number[] => {
  ...
  if (Array.isArray(valor)) {
    return valor.map((item) => Number(item)).filter((item) => !Number.isNaN(item) && item > 0)
  }
  ...
  if (typeof valor === 'string') {
    try {
      ...
      if (Array.isArray(parsed)) {
        return parsed.map((item) => Number(item)).filter((item) => !Number.isNaN(item) && item > 0)
      }
    } catch {
      const numero = Number(valor)
...
const subirImagenACloudinary = async (file: Express.Multer.File): Promise<string> => {
...
export const editarMultimediaPublicacionService = async (
  ...
  const videosFinales =
    videosUrls.length > 0 ? videosUrls.slice(0, 2) : videoUrlLegacy ? [videoUrlLegacy] : []
  ...
  await crearMultimediaRepository([...nuevasImagenesDesdeArchivos, ...nuevasImagenesDesdeBase64])
  ...
  const multimediaActualizada = await buscarMultimediaPublicacionRepository(publicacionId)
...
export const cancelarPublicidadService = async (publicacionId: number, usuarioId: number) => {
  if (Number.isNaN(publicacionId) || publicacionId <= 0) {
    throw new Error('ID_INVALIDO')
  }
