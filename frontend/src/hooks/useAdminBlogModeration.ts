...
function loadStoredBlogs() {
  ...
  if (!rawBlogs) {
    ...
    localStorage.setItem(ADMIN_BLOGS_STORAGE_KEY, JSON.stringify(ADMIN_MODERATION_BLOGS_MOCK))
...
export function useAdminBlogModeration() {
  const [blogs, setBlogs] = useState<AdminModerationBlog[]>(ADMIN_MODERATION_BLOGS_MOCK)
    ...
    setBlogs((currentBlogs) => {
      const updatedBlogs = currentBlogs.map((blog) => {
        ...
        return {
          ...blog,
          status: nextStatus,
          rejectionComment: nextStatus === 'RECHAZADO' ? (rejectionComment?.trim() ?? '') : null,
          reviewedAt: new Date().toISOString()
        }
      })

      // TODO: persistir aprobacion y rechazo con endpoints reales cuando exista backend.
      localStorage.setItem(ADMIN_BLOGS_STORAGE_KEY, JSON.stringify(updatedBlogs))
