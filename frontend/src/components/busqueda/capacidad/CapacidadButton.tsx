...
export function CapacidadButton({
  variant = "map",
  isActive = false,
  onClick,
}: CapacidadButtonProps) {
    ...
    <div className="shrink-0 relative">
      <button type="button" onClick={onClick} className={buttonStyles}>
        <div className="flex items-center gap-2">
          <Users
            className={`w-4 h-4 ${isActive ? "text-white" : "text-stone-500 dark:text-stone-400"}`}
          />
          <span>Capacidad</span>
        </div>
        <ChevronDown
          className={`w-4 h-4 transition-transform ${isActive ? "rotate-180 text-white" : "text-stone-400 dark:text-stone-400 opacity-70"}`}
        />
      </button>
    </div>
  );
