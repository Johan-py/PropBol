...
export default function ComparatorModal({
  ...
  useEffect(() => {
    setProperties((prev) =>
      prev.filter((p) => selectedIds.includes(String(p.id))),
    );
  }, [selectedIds]);
                ...
                <thead>
                  <tr>
                    ...
                    {properties.map((prop) => (
                      <th
                        key={prop.id}
                        className="border-b border-stone-200 p-4 w-72 bg-white align-top relative group"
                      >
                        <button
                        ...
                        <div className="aspect-[4/3] rounded-lg overflow-hidden bg-stone-100 mb-3 relative">
                          <img
                            src={
                              prop.publicaciones?.[0]?.multimedia?.[0]?.url ||
                ...
                <tbody className="divide-y divide-stone-100">
                  ...
                  <tr className="bg-stone-50">
                    <td
                      ...
                      className="py-2 px-4 border-y border-stone-200 sticky left-0 z-20"
                    >
                  ...
                  <tr className="hover:bg-stone-50/50 transition-colors">
                    <td className="sticky left-0 z-20 bg-white border-r border-stone-200 p-4 text-sm font-semibold text-slate-600 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                      Precio
                    </td>
                    {properties.map((prop) => (
                      <td
                        key={prop.id}
                        className="p-4 text-lg font-bold text-[#ea580c]"
                      >
                        $ {prop.precio?.toLocaleString("en-US")}
                      </td>
                    ))}
                  </tr>
                  <tr className="hover:bg-stone-50/50 transition-colors">
                  ...
                  <tr className="hover:bg-stone-50/50 transition-colors">
                    ...
                    {properties.map((prop) => (
                      <td key={prop.id} className="p-4 text-sm">
                        {renderVal(prop.antiguedad, "años")}
                      </td>
                    ))}
                  </tr>
                        ...
                        <td key={prop.id} className="p-4">
                          {tags.length > 0 ? (
                            <div className="flex flex-wrap gap-1.5">
                              {tags.map((t) => (
                                <span
                                  key={`tag-${prop.id}-${t.etiqueta.nombre}`}
                                  className="px-2.5 py-1 bg-stone-100 text-stone-600 border border-stone-200 rounded-lg text-[11px] font-bold uppercase tracking-wide"
                                >
                                  {t.etiqueta.nombre}
                                </span>
                              ))}
                        ...
                        <td key={prop.id} className="p-4">
                          {amenidades.length > 0 ? (
                            <div className="flex flex-wrap gap-1.5">
                              {amenidades.map((a) => (
                                <span
                                  key={`amenity-${prop.id}-${a.amenidad.nombre}`}
                                  className="px-2.5 py-1 bg-orange-50 text-[#ea580c] border border-orange-200 rounded-lg text-xs font-medium"
                                >
                                  {a.amenidad.nombre}
                                </span>
                              ))}
