...
export default function PriceFilterSidebar({
  ...
  const [moneda, setMoneda] = useState<"BOB" | "USD">(
    (searchParams.get("currency") as "BOB" | "USD") || "USD",
  );
  ...
  useEffect(() => {
    setDisplayMin(minPrice ? formatCurrency(minPrice) : "");
  }, [minPrice]);
  useEffect(() => {
    setDisplayMax(maxPrice ? formatCurrency(maxPrice) : "");
  }, [maxPrice]);
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-stone-600 w-20 text-right">
              {maxPrice
                ...
                : `${(LIMITE_MAX / 1000).toLocaleString("de-DE")}K`}{" "}
              {moneda}
            </span>
          </div>
        </div>
        ...
        {filtroAplicado && totalResultados === 0 && (
          <div className="flex flex-col items-center gap-2 py-3 text-center">
            ...
            <p className="text-xs text-stone-400">
              No se encontraron propiedades dentro del rango de precio
              seleccionado
            </p>
          </div>
        )}
