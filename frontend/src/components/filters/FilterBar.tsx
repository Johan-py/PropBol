...
export default function FilterBar({
  ...
  isZonaFilterActive = false,
  isOfertaActive = false,
  onToggleOferta,
  isEtiquetasFilterActive = false,
  onOpenEtiquetasFilter,
}: FilterBarProps) {
  ...
  useEffect(() => {
    ...
    if (urlTipo && urlTipo !== "CUALQUIER TIPO") {
      const match = propertyTypes.find(
        (pt) => pt.label.toUpperCase() === urlTipo.toUpperCase(),
      );
  ...
  const activeFilters = useMemo(() => {
    ...
    const removeParam = (keys: string[], customAction?: () => void) => {
      ...
      router.push(
        `/busqueda_mapa${params.toString() ? `?${params.toString()}` : ""}`,
      );
    };
    ...
    if (tipo && tipo !== "CUALQUIER TIPO") {
      const tipoLabel =
        propertyTypes.find((pt) => pt.label.toUpperCase() === tipo)?.label ||
        tipo;
      filters.push({
        id: "tipo",
        label: tipoLabel,
        onRemove: () =>
          removeParam(["tipoInmueble"], () =>
            setTipoInmueble("Cualquier tipo"),
          ),
      });
    }
    ...
    if (query)
      filters.push({
        ...
        label: `Zona: ${query}`,
        onRemove: () =>
          removeParam(["query", "lat", "lng", "radius"], () => {
            setUbicacionTexto("");
            setCoords({});
          }),
      });
    ...
    const depId = params.get("departamentoId");
    const provId = params.get("provinciaId");
    const munId = params.get("municipioId");
    const zonaId = params.get("zonaId");
    const barId = params.get("barrioId");
    ...
    // -- Superficie --
    const minS = params.get("minSuperficie");
    const maxS = params.get("maxSuperficie");
    if (minS || maxS) {
      let lbl = "Sup: ";
      ...
      filters.push({
        id: "superficie",
        label: lbl,
        onRemove: () => removeParam(["minSuperficie", "maxSuperficie"]),
      });
    }

    // -- Cuartos y Baños --
    const minD = params.get("dormitoriosMin");
    const maxD = params.get("dormitoriosMax");
    if (minD || maxD)
      filters.push({
        id: "cuartos",
        label: `Cuartos: ${minD || 0} a ${maxD || "+"}`,
        onRemove: () => removeParam(["dormitoriosMin", "dormitoriosMax"]),
      });

    const minB = params.get("banosMin");
    const maxB = params.get("banosMax");
    if (minB || maxB)
      filters.push({
        id: "banos",
        label: `Baños: ${minB || 0} a ${maxB || "+"}`,
        onRemove: () => removeParam(["banosMin", "banosMax"]),
      });
    ...
    amenities.forEach((a) =>
      filters.push({
        id: `amenity-${a}`,
        label: `Amenidad: ${AMENITIES_MAP[a] || a}`,
        onRemove: () => {
    ...
    const etiquetasNombres: Record<string, string> = (() => {
      try {
        return JSON.parse(
          sessionStorage.getItem("propbol_etiquetas_nombres") || "{}",
        );
      } catch {
        return {};
      }
    ...
    labels.forEach((l) =>
      filters.push({
        id: `label-${l}`,
        label: etiquetasNombres[l] || `Etiqueta ${l}`,
        onRemove: () => {
        ...
        <div className="flex flex-col gap-3 w-full max-w-screen-2xl mx-auto">
          ...
          <div className="flex flex-wrap md:flex-nowrap items-center w-full gap-3 relative z-[100] !overflow-visible">
            ...
            <div className="shrink-0 w-full md:w-auto relative z-10">
              <button
                type="submit"
                className="w-full md:w-auto h-[42px] px-8 bg-[#d97706] hover:bg-[#b95e00] text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-md transition-all active:scale-95"
              >
                <SearchIcon size={18} />
              </button>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 relative z-[80] justify-center md:justify-start">
            ...
            >
              <MapPin
                className={`w-4 h-4 ${isZonaFilterActive ? "text-white" : "text-stone-500 dark:text-stone-400"}`}
              />
              <span>Zona</span>
            </button>
            ...
            >
              <DollarSign
                className={`w-4 h-4 ${isPriceFilterActive ? "text-white" : "text-stone-500 dark:text-stone-400"}`}
              />
              <span>Precio</span>
              <ChevronDown
                className={`w-4 h-4 ${isPriceFilterActive ? "text-white" : "text-stone-400 dark:text-stone-400"}`}
              />
            </button>
            ...
            >
              <Maximize
                className={`w-4 h-4 ${isSuperficieFilterActive ? "text-white" : "text-stone-500 dark:text-stone-400"}`}
              />
              <span>Metros</span>
              <ChevronDown
                className={`w-4 h-4 ${isSuperficieFilterActive ? "text-white" : "text-stone-400 dark:text-stone-400"}`}
              />
            </button>

            <button
              type="button"
              onClick={() => onOpenEtiquetasFilter?.()}
              className={`h-[38px] flex items-center gap-2 px-4 rounded-full border text-sm font-medium shadow-sm transition-all focus:outline-none shrink-0 ${isEtiquetasFilterActive ? "bg-[#d97706] text-white border-[#d97706] dark:bg-[#E87C1E] dark:border-[#E87C1E]" : "bg-white dark:bg-stone-800 text-stone-600 dark:text-stone-300 border-stone-200 dark:border-stone-700 hover:border-[#d97706] dark:hover:border-[#E87C1E] dark:hover:bg-stone-700"}`}
            >
              <Tag
                className={`w-4 h-4 ${isEtiquetasFilterActive ? "text-white" : "text-stone-500 dark:text-stone-400"}`}
              />
              <span>Etiquetas</span>
              <ChevronDown
                className={`w-4 h-4 ${isEtiquetasFilterActive ? "text-white" : "text-stone-400 dark:text-stone-400"}`}
              />
            </button>
            ...
            <button
              ...
              onClick={async (e) => {
                ...
                modoInmueble.forEach((m) =>
                  cleanParams.append("modoInmueble", m),
                );
                ...
                if (token) {
                  try {
                    const res = await fetch(
                      `/api/inmuebles/recomendados?${cleanParams.toString()}`,
                      {
                  ...
                  try {
                    ...
                    console.error(
                      "Error cargando populares para visitante:",
                      error,
                    );
            ...
            >
              <Award
                className={`w-4 h-4 ${searchParams?.get("orden") === "recomendados" ? "text-white" : "text-stone-500"}`}
              />
            ...
            >
              <BarChart2
                className={`w-4 h-4 ${isCompareMode ? "text-white" : "text-stone-500"}`}
              />
              <span>
                Comparar{" "}
          ...
          {activeFilters.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 bg-white/60 dark:bg-stone-950/60 backdrop-blur-sm border border-stone-200 dark:border-stone-700 rounded-xl p-2.5 w-full shadow-inner min-h-[48px]">
              <span className="text-[11px] text-stone-500 font-bold uppercase tracking-wider ml-2 mr-1">
                Activos:
              </span>
              ...
              {activeFilters.map((filter) => (
                <div
                  key={filter.id}
                  className="group flex items-center gap-1.5 bg-[#fdf3e7] dark:bg-orange-500/20 border border-orange-200 dark:border-orange-500/30 text-orange-800 dark:text-orange-300 px-3 py-1.5 rounded-lg text-xs font-semibold shadow-sm transition-all hover:bg-orange-100 dark:hover:bg-orange-500/40 animate-in fade-in zoom-in duration-200"
                >
