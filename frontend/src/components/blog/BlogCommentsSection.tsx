...
const CommentItem = ({
  ...
  return (
    <div
      className={level > 0 ? "border-l-2 border-stone-200 pl-6 sm:pl-10" : ""}
    >
      <article
        className={`relative rounded-[24px] bg-[#F8F9FA] p-4 shadow-sm border border-gray-100 sm:p-6 transition-all hover:shadow-md ${(comment as any).isNewSocket ? "border-l-4 border-l-orange-500" : ""}`}
      >
              ...
              <div className="min-w-0">
                <div className="flex flex-col items-start gap-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    {(comment as any).isNewSocket && (
                      <span className="bg-orange-100 text-orange-600 text-[10px] font-extrabold px-2 py-0.5 rounded-md uppercase">
                        Nuevo
                      </span>
                    )}
                  </div>
  ...
  return (
    <section className="mt-16 bg-stone-50 py-12 sm:mt-20 sm:py-16">
              ...
              <div className="mt-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="inline-flex min-h-[48px] items-center justify-center rounded-xl bg-[#3D3D3D] px-8 text-[12px] font-bold uppercase tracking-[0.16em] text-white transition hover:bg-black disabled:cursor-not-allowed disabled:bg-stone-300 shadow-md shadow-black/5"
                  >
                    {isSubmitting
                      ? "Publicando..."
                      : activeEdition
      ...
      <DeleteConfirmationModal
        ...
        onConfirm={async () => {
          await requestDelete(commentToDelete!);
          setCommentToDelete(null); // Cerramos el modal después de borrar
        }}
