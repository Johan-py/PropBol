...
export default function BlogSharePlaceholder({
  ...
  const getTitle = () =>
    title ||
    (typeof document !== "undefined" ? document.title : "Blog PropBol");
    ...
    try {
      ...
      const canvas = await html2canvas(articleElement, {
        ...
        onclone: (clonedDoc) => {
          ...
          // Ajustar el grid
          const grid = clonedDoc.querySelector(
            ".blog-grid-container",
          ) as HTMLElement;
          if (grid) {
      ...
      doc.save(
        `${title?.replace(/\s+/g, "-").toLowerCase() || "blog-completo"}.pdf`,
      );
    } catch (error) {
  ...
  const shareToGmail = () => {
    window.open(
      `https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent(getTitle())}&body=${encodeURIComponent("¡Hola! Te comparto este artículo que me pareció interesante:\n\n" + getUrl())}`,
      "_blank",
    );
  };
  ...
  const shareToWhatsApp = () => {
    const text = encodeURIComponent(
      `¡Hola! Te comparto este artículo que me pareció interesante:\n\n${getTitle()}\n${getUrl()}`,
    );
  ...
  const shareToX = () => {
    const text = encodeURIComponent(
      `¡Hola! Te comparto este artículo que me pareció interesante de PropBol:\n\n${getTitle()}\n`,
    );
    window.open(
      `https://twitter.com/intent/tweet?url=${encodeURIComponent(getUrl())}&text=${text}`,
      "_blank",
    );
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between w-full gap-5">
          <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto w-full lg:w-auto pr-2 lg:pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <button
              onClick={shareToX}
              className="flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 transition-colors duration-200 group shrink-0"
            ...
            >
              <svg
                viewBox="0 0 24 24"
                className="w-5 h-5 opacity-90 group-hover:opacity-100 transition-opacity text-[#433527] dark:text-stone-300"
                fill="none"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
          ...
          <div className="relative w-full lg:w-auto lg:shrink-0" ref={menuRef}>
            <button
              ...
              className={`flex items-center justify-between gap-3 h-11 px-4 sm:h-12 sm:px-5 w-full lg:w-auto rounded-xl border transition-all duration-300 group whitespace-nowrap ${
                isDownloadOpen
            ...
            >
              <div className="flex items-center gap-2">
                {isGenerating ? (
                  ...
                  <svg
                    viewBox="0 0 24 24"
                    className={`w-[18px] h-[18px] ${isDownloadOpen ? "text-stone-100" : "text-[#433527] dark:text-stone-300"}`}
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinecap="round"
              ...
              <svg
                viewBox="0 0 24 24"
                className={`w-4 h-4 transition-transform duration-300 ${isDownloadOpen ? "rotate-180 text-white" : "text-stone-400 group-hover:text-stone-600"}`}
                fill="none"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!articleElement) {
                        alert(
                          "No se encontró el contenido principal para descargar.",
                        );
                        setIsGenerating(false);
                        return;
                      }
                      ...
                      const canvas = await html2canvas(articleElement, {
                        ...
                        onclone: (clonedDoc) => {
                          ...
                          const elementsToHide =
                            clonedDoc.querySelectorAll(".no-capture");
                          elementsToHide.forEach((el) => {
                            (el as HTMLElement).style.display = "none";
                          });
                      ...
                      alert(
                        "Hubo un error al intentar descargar el blog como imagen.",
                      );
                    } finally {
                      setIsGenerating(false);
                    }
