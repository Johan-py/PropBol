  ...
  if (status === 'APROBADO') {
    return (
      <div className="rounded-[28px] border border-emerald-200 bg-emerald-50 px-6 py-5 text-emerald-800">
        <p className="text-xs font-semibold uppercase tracking-[0.25em]">Estado actual</p>
        <h2 className="mt-2 text-2xl font-bold">Articulo publicado</h2>
        <p className="mt-2 text-sm leading-7">
          Este blog ya fue aprobado en la demo, por lo que no volvera a aparecer en la pestana de
          pendientes.
        </p>
      </div>
    )
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="rounded-[32px] border border-stone-200 bg-white px-6 py-12 text-center shadow-sm">
          <h1 className="text-3xl font-bold text-stone-900">Blog no encontrado</h1>
          <p className="mt-3 text-base leading-7 text-stone-600">
            El articulo solicitado no existe en la demo o fue removido del almacenamiento local.
          </p>
        ...
        <div className="mt-10 rounded-[36px] border border-stone-200 bg-white p-5 shadow-[0_24px_80px_rgba(28,25,23,0.08)] sm:p-8 lg:p-10">
          ...
          <div className="mt-10 grid gap-5 border-b border-stone-200 pb-8 text-sm text-stone-600 sm:grid-cols-3">
            <div>
              ...
              <p className="mt-2 text-lg font-semibold text-stone-900">{blog.authorName}</p>
              <p>{blog.authorRole}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-stone-400">
            ...
            <div>
              ...
              <p className="mt-2 text-lg font-semibold text-stone-900">{blog.readingTime}</p>
            </div>
          </div>
            ...
            {blog.status === 'PENDIENTE' ? (
              <div className="space-y-6">
                <div className="flex flex-col gap-2">
                  ...
                  <h2 className="text-3xl font-bold text-stone-900">Revisa antes de publicar</h2>
                  <p className="max-w-2xl text-base leading-7 text-stone-600">
                    Si rechazas el articulo, agrega un comentario claro para que el autor sepa que
                    debe mejorar.
                  </p>
                </div>
              ...
              <StatusBanner status={blog.status} rejectionComment={blog.rejectionComment} />
            )}
          </div>
        </div>
      </article>
