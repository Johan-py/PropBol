...
export default function FilterPanel() {
  ...
  if (loading) return <aside className="w-80 p-6 text-gray-500 font-medium italic font-inter">Sincronizando...</aside>;
  ...
  return (
    <aside className="w-full md:w-80 bg-white p-6">
      ...
      <section className="mt-4">
        <h3 className="text-xl font-bold text-black mb-3 border-b-2 border-black inline-block font-inter tracking-tight">Alquileres</h3>
        <div className={`flex flex-col gap-2 mt-2 ${rentalsLogic.viewLevel > 2 ? 'max-h-60 overflow-y-auto pr-2' : ''}`}>
          ...
          {rentalsLogic.viewLevel < 3 && rentalsData.length > 2 ? (
            <button onClick={rentalsLogic.handleSeeMore} className="text-sm text-orange-400 underline mt-1 w-fit font-bold font-inter">
              {salesLogic.viewLevel === 1 ? 'Ver más >' : 'Mostrar todo >'}
            </button>
          ) : rentalsData.length > 2 && (
            <button onClick={rentalsLogic.handleSeeLess} className="text-sm text-orange-400 underline mt-1 w-fit ml-auto font-bold font-inter">
              {'<'} Ver menos
            </button>
          )}
        </div>
      ...
      <section className="mt-8">
        <h3 className="text-xl font-bold text-black mb-3 border-b-2 border-black inline-block font-inter tracking-tight">En venta</h3>
        <div className={`flex flex-col gap-2 mt-2 ${salesLogic.viewLevel > 2 ? 'max-h-60 overflow-y-auto pr-2' : ''}`}>
          ...
          {salesLogic.viewLevel < 3 && salesData.length > 2 ? (
            <button onClick={salesLogic.handleSeeMore} className="text-sm text-orange-400 underline mt-1 w-fit font-bold font-inter">
              {rentalsLogic.viewLevel === 1 ? 'Ver más >' : 'Mostrar todo >'}
            </button>
          ) : salesData.length > 2 && (
            <button onClick={salesLogic.handleSeeLess} className="text-sm text-orange-400 underline mt-1 w-fit ml-auto font-bold font-inter">
              {'<'} Ver menos
            </button>
          )}
        </div>
      ...
      {/* SECCIÓN: INMUEBLES */}
      <section className="mt-8">
        <h3 className="text-xl font-bold text-black mb-3 border-b-2 border-black inline-block font-inter tracking-tight">Inmuebles</h3>
        <div className={`flex flex-col gap-2 mt-2 ${typesLogic.viewLevel > 2 ? 'max-h-60 overflow-y-auto pr-2' : ''}`}>
          ...
          {typesLogic.viewLevel < 3 && typesData.length > 2 ? (
            <button onClick={typesLogic.handleSeeMore} className="text-sm text-orange-400 underline mt-1 w-fit font-bold font-inter">
              {typesLogic.viewLevel === 1 ? 'Ver más >' : 'Mostrar todo >'}
            </button>
          ) : typesData.length > 2 && (
            <button onClick={typesLogic.handleSeeLess} className="text-sm text-orange-400 underline mt-1 w-fit ml-auto font-bold font-inter">
              {'<'} Ver menos
            </button>
          )}
        </div>
