...
export default function VisualFiltersSection() {
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        const payload: BackendResponse = (json as any).data ?? json;

        setAlquileres(
          mergeDepartamentos(DEPARTAMENTOS_BASE, payload.rentals ?? []),
        );
        ...
        <div className="w-full max-w-[1100px] animate-pulse space-y-8">
          ...
          <div className="flex gap-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="min-w-[200px] h-[200px] bg-gray-200 rounded-xl"
              />
            ))}
          </div>
          <div className="h-4 w-24 bg-gray-200 rounded" />
          <div className="flex gap-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="min-w-[160px] h-[160px] bg-gray-200 rounded-xl"
              />
            ))}
          </div>
        </div>
  ...
  const alquilerItems = alquileres
    .map((item) => ({
  ...
  return (
    <section
      ...
      className="w-full px-4 md:px-8 py-8 flex justify-center"
    >
