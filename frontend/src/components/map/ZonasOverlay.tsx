...
function dimensionarEtiqueta(
  ...
  const zoomProportional = 0.7 + (zoomOffset / 8) * 0.4; // Rango: 0.36 a ~1.02
  let scale = Math.max(0.4, Math.min(1.12, zoomProportional + selectedBoost));
  ...
  let lineHeight = Math.round(fontSize * 1.2 * 10) / 10;
  ...
  const factorAnchoWrap = isSelected ? 0.7 : 0.75;
  ...
  let maxCharsPorLinea = Math.max(
    4,
    Math.floor(anchoDisponible / charWidthEstimado),
  );
  let lineasTexto = construirLineasEtiqueta(nombre, maxCharsPorLinea);
  let largoLineaMasLarga = lineasTexto.reduce(
  ...
  const widthObjetivo = Math.round(
    largoLineaMasLarga * (fontSize * 0.56) + paddingX * 2 + 18,
  );
  ...
  const finalWidthObjetivo = Math.round(
    largoLineaMasLarga * (fontSize * 0.56) + paddingX * 2 + 18,
  );
  ...
  if (isSelected && lineas > 1) {
    ...
    if (totalHeightNeeded > maxHeightReasonable) {
      const reductionFactor = Math.min(
        0.9,
        maxHeightReasonable / totalHeightNeeded,
      );
...
function construirLineasEtiqueta(
  nombre: string,
  maxCharsPorLinea: number,
): string[] {
  const texto = nombre.replace(/\s+/g, " ").trim();
...
function labelIcon(
  ...
  const maxChars = isSelected
    ? MAX_LABEL_CHARS_SELECTED * 1.3
    : MAX_LABEL_CHARS;
  ...
  const color = isSelected
    ? colorConfig.labelColorSelected
...
function ZonaInteractiva({
  ...
  const tipoZona: TipoZona =
    zona.tipo || (zona.id < 0 ? "personalizada" : "predefinida");
  const colorConfig = COLORES[tipoZona];
  ...
  return (
    <Fragment>
      <Polygon
        ...
        pathOptions={{
          color: selected
            ? colorConfig.borderActive
            : colorConfig.borderInactive,
          weight: selected ? 2 : 1.8,
          dashArray: selected ? "6,6" : undefined,
          fillColor: selected
            ? colorConfig.fillActive
            : colorConfig.fillInactive,
          fillOpacity: selected
            ? colorConfig.fillOpacityActive
            : colorConfig.fillOpacityInactive,
          lineJoin: "round",
          lineCap: "round",
        }}
        ...
        eventHandlers={{
          add: () => {
            const element =
              polygonRef.current?.getElement() as SVGPathElement | null;
            if (!element) return;
