...
>>>>>>> b911b2bf644730e8e9778ee2b2988ca63e6edaa2
...
<<<<<<< HEAD
  // Restaurar bloqueo desde localStorage al montar
  useEffect(() => {
  ...
  // Countdown: un tick por segundo mientras haya bloqueo
  useEffect(() => {
...
>>>>>>> b911b2bf644730e8e9778ee2b2988ca63e6edaa2
    try {
      setErrorMessage("");
      if (!API_URL) { setErrorMessage("No se configuró NEXT_PUBLIC_API_URL en el frontend."); return; }
      const token = localStorage.getItem("token");
      if (!token) { setErrorMessage("No se encontró la sesión del usuario."); return; }
      setIsSendingCode(true);
      const response = await fetch(`${API_URL}/api/security/deactivate-account/send-code`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      });
      const data = await response.json();
      if (!response.ok) { setErrorMessage(data.message || "No se pudo enviar el código."); return; }
      setEmailVerificationToken(data.verificationToken || "");
      setShowPasswordModal(false);
      setShowEmailCodeModal(true);
      setEmailCode("");
  ...
  const handleDeactivateAccount = async () => {
    // Guardia doble: no ejecutar si está bloqueado
    ...
    if (!trimmedPassword) {
      setErrorMessage("La contraseña es obligatoria y no puede contener solo espacios en blanco.");
      return;
    }
    if (password.length > MAX_PASSWORD_LENGTH) {
      setErrorMessage(`La contraseña no puede superar ${MAX_PASSWORD_LENGTH} caracteres.`);
      return;
    }
    if (!API_URL) { setErrorMessage("No se configuró NEXT_PUBLIC_API_URL en el frontend."); return; }
    const token = localStorage.getItem("token");
...
<<<<<<< HEAD
    ...
    try {
      const response = await fetch(`${API_URL}/api/security/deactivate-account`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
...
>>>>>>> b911b2bf644730e8e9778ee2b2988ca63e6edaa2
      ...
      if (!response.ok) {
        // 429 = backend bloqueó al usuario → activar bloqueo visual
        if (response.status === 429) {
          triggerLockout();
          setErrorMessage("Demasiados intentos fallidos. El campo y el botón estarán bloqueados durante 15 minutos.");
          return;
        }
...
<<<<<<< HEAD
      ...
      if (!trimmedCode) { setEmailCodeError("El código es obligatorio."); return; }
      if (!/^\d{6}$/.test(trimmedCode)) { setEmailCodeError("El código debe tener exactamente 6 dígitos."); return; }
      if (!emailVerificationToken) { setEmailCodeError("No se encontró la verificación del código. Solicita uno nuevo."); return; }
      if (!API_URL) { setEmailCodeError("No se configuró NEXT_PUBLIC_API_URL en el frontend."); return; }
      const token = localStorage.getItem("token");
      if (!token) { setEmailCodeError("No se encontró la sesión del usuario."); return; }
      setIsVerifyingCode(true);
      const response = await fetch(`${API_URL}/api/security/deactivate-account/verify-code`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ codigo: trimmedCode, verificationToken: emailVerificationToken }),
      });
      const data = await response.json();
      if (!response.ok) { setEmailCodeError(data.message || "No se pudo validar el código."); return; }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-xl border border-neutral-300 bg-white p-5 shadow-xl">
            ...
            <div className="space-y-2">
              ...
              <div className={`flex h-11 items-center rounded-lg border px-3 transition-colors ${isLocked ? "border-red-300 bg-red-50" : "border-neutral-300 bg-white"}`}>
                <input
                  id="current-password"
...
>>>>>>> b911b2bf644730e8e9778ee2b2988ca63e6edaa2
                <button
                  type="button"
                  onClick={() => { if (!isLocked) setShowPassword((c) => !c); }}
                  disabled={isLocked}
...
<<<<<<< HEAD
                Si te registraste con Google, Discord u otro método externo, puedes verificar mediante correo.
=======
          ...
          <div className="w-full max-w-md rounded-xl border border-neutral-300 bg-white p-5 shadow-xl">
            <div className="mb-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-neutral-600">
                  Ingresa el código enviado al correo asociado a tu cuenta para desactivarla.
                </p>
              </div>
            </div>
            <div className="space-y-2">
<<<<<<< HEAD
              <label htmlFor="email-code" className="text-sm font-medium text-neutral-700">
...
<<<<<<< HEAD
            {emailCodeError && (
              <p className="mt-3 text-sm font-medium text-red-600">{emailCodeError}</p>
            )}
            {emailCodeSuccess && (
              <p className="mt-3 text-sm font-medium text-green-600">{emailCodeSuccess}</p>
            )}
=======
