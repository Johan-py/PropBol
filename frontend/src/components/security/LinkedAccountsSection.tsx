...
function SocialCard({ account, actionLoadingId, onLink, onOpenUnlinkModal }: SocialCardProps) {
  return (
    <article className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button
          type="button"
          onClick={() =>
            account.status === 'vinculado' ? onOpenUnlinkModal(account.id) : onLink(account.id)
          }
...
export default function LinkedAccountsSection() {
        ...
        prev.map((account) =>
          account.id === id
            ? {
                ...
                status: 'vinculado',
                linkedEmail:
                  id === 'facebook' ? 'usuario.facebook@mock.com' : 'usuario.discord@mock.com'
              }
            : account
        )
      )
  ...
  const confirmUnlink = () => {
    ...
    const linkedAccountsCount = accounts.filter((account) => account.status === 'vinculado').length

    if (linkedAccountsCount <= 1) {
      setErrorMessage('No puedes desvincular el único método de acceso disponible.')
      setAccountToUnlink(null)
      return
    }
