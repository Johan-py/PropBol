...
export default function TagsPropiedad({
  ...
  useEffect(() => {
    ...
    const filtradas = catalogoTags
      ...
      .filter(
        (t) =>
          !tags.some((tag) => tag.toLowerCase() === t.nombre.toLowerCase()),
      )
      .slice(0, 6);
  ...
  const agregarTag = (nombre: string) => {
    ...
    if (valor.length < MIN_CARACTERES || valor.length > MAX_CARACTERES) {
      setError(
        `El tag debe tener entre ${MIN_CARACTERES} y ${MAX_CARACTERES} caracteres`,
      );
      return;
    }
  ...
  return (
    <div className="mt-4 rounded-2xl border border-neutral-300 bg-[#f5ede2] p-6">
      ...
      <p className="mb-4 text-sm text-neutral-600">
        Agrega palabras clave para que los clientes encuentren tu inmueble más
        fácilmente. Presiona Enter o haz clic en "+ Agregar".
      </p>
      ...
      <p className="mb-4 text-xs text-neutral-500">
        Puedes agregar hasta {MAX_TAGS} tags. Cada tag debe tener entre{" "}
        {MIN_CARACTERES} y {MAX_CARACTERES} caracteres.
      </p>
