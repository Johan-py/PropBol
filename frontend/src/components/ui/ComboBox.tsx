...
export function ComboBox({
  ...
  return (
    <div
      className={`flex flex-col w-full font-inter ${label ? "gap-2" : ""}`}
      ref={comboBoxRef}
    >
      {label && (
        <label className="text-sm font-medium text-stone-900 dark:text-stone-200">
          {label}
        </label>
      )}
      ...
      <div className="relative group">
        ...
        >
          <span
            className={`text-sm ${selected ? "text-stone-900 dark:text-stone-100 font-medium" : "text-stone-500 dark:text-stone-400"}`}
          >
