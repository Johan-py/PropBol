    ...
    >
      ...
      <span
        className={`font-semibold transition-all duration-300 ${
          isHovered
            ? "text-base text-[#ea580c]"
            : "text-xs text-gray-700 dark:text-stone-300"
        }`}
      >
        {precioFormateado}
      </span>
      ...
      <div className="flex flex-col overflow-hidden min-w-0">
        <span className="text-xs font-semibold text-gray-800 dark:text-stone-200 truncate">
          {title}
        </span>
        <span className="text-[11px] text-gray-600 dark:text-stone-400 truncate">
          {size}
        </span>
      ...
      <div
        ...
        role="navigation"
        aria-label="Navegar a ubicacion de la propiedad"
      >
        <ComoLlegarButton
          lat={lat}
          lng={lng}
          variant="table"
          disabled={isCompareMode}
          data-testid="como-llegar-row"
        />{" "}
        {/* Bloqueamos si compara */}
      </div>
      <div className="flex justify-center">
        <ContactButton
