...
export default function MapaZonas({
  ...
  const [propiedadesPorZona, setPropiedadesPorZona] = useState<
    Record<number, Propiedad[]>
  >({});
      ...
      for (const zonaId of zonasConPropiedades) {
        try {
          const response = await fetch(
            `${API_URL}/api/perfil/zonas/${zonaId}/propiedades`,
            {
          ...
          console.error(
            `Error cargando propiedades para zona ${zonaId}:`,
            error,
          );
      ...
      const handleTouchEnd = (e: TouchEvent) => {
        ...
        if (maxTouchCount === 2) {
          ...
          if (timeSinceLast < 350) {
            map.flyTo(map.getCenter(), Math.max(map.getZoom() - 1, 1), {
              duration: 0.35,
            });
          }
        }
            ...
            <Popup>
              <div className="text-sm min-w-[200px]">
                ...
                {tienePropiedadesVisibles ? (
                    ...
                    {isLoadingPropiedades ? (
                      ...
                      <>
                        <p className="text-xs text-amber-600 mt-2 font-medium">
                          {propiedadesDeEstaZona.length} propiedad(es)
                          encontrada(s)
                        </p>
                        <div className="mt-2 max-h-40 overflow-y-auto">
                          {propiedadesDeEstaZona.slice(0, 3).map((prop) => (
                            <div
                              key={prop.id}
                              className="text-xs text-stone-600 border-t border-stone-100 mt-1 pt-1"
                            >
                              <p className="font-medium">{prop.titulo}</p>
                              <p className="text-amber-600">
                                {prop.tipo_accion === "ALQUILER"
                                  ? `$${prop.precio}/mes`
                                  : `$${prop.precio}`}
                              </p>
                            </div>
                  ...
                  <p className="text-xs text-stone-400 mt-2">
                    Activa "Mostrar propiedades" en la tarjeta para ver las
                    propiedades dentro de esta zona.
                  </p>
                )}
              </div>
            </Popup>
            ...
            <Popup>
              <div className="text-sm min-w-[220px]">
                ...
                <p className="text-xs text-stone-500 mt-0.5">
                  {prop.superficie_m2}m² | {prop.nro_cuartos || "?"} hab |{" "}
                  {prop.nro_banos || "?"} baños
                </p>
