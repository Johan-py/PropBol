...
export default function UserPreferencesModal({ isOpen, onClose, onSuccess }: UserPreferencesModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. ENVÍO AL BACKEND (Preparado para la integración)
    try {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
      const response = await fetch(`${apiUrl}/api/perfil/usuario`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        },
        body: JSON.stringify({ genero, fecha_nacimiento: fechaNacimiento }),
      });
      ...
      if (response.ok) {
        ...
        setErrorMsg(errorData.message || 'Error al guardar los datos en el servidor.');
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden relative border border-stone-100 animate-in zoom-in duration-300">
        ...
        <div className="p-8">
          ...
          <p className="text-[#78716c] mb-6 text-sm text-center">
            Ajusta estos detalles para personalizar tu experiencia.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Campo Género - ILUMINADO EN ÁMBAR */}
            ...
            {/* Campo Fecha - ILUMINADO EN ÁMBAR */}
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-stone-700 block">
                Fecha de Nacimiento
              </label>
            ...
            <div className="flex justify-end gap-3 font-semibold mt-8">
              ...
              >
                {isLoading ? <span className="animate-pulse">Guardando...</span> : 'Guardar Perfil'}
              </button>
            </div>
          </form>
        </div>
