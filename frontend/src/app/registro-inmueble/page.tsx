...
export default function MiRegistroPage() {
  ...
  const erroresHU5: ErrorValidacion[] =
    campoError && estado === "error" && mensajeError
      ? [
          {
            campo: campoError as any,
  ...
  const guardarPropiedad = async () => {
    ...
    if (
      pois.some((poi) => !poi.nombre.trim() || poi.nombre.trim().length < 3)
    ) {
      setMensajeError("LAS REFERENCIAS DEBEN TENER MÍNIMO 3 CARACTERES");
      setEstado("error");
      return;
    }
    ...
    try {
      ...
      for (const poi of pois) {
        ...
        if (!responsePoi.ok) {
          setMensajeError("ERROR AL GUARDAR REFERENCIAS");
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const soloNumeros = limpiarSoloNumeros(
                            e.target.value,
                          );
                          manejarCambio({
                            ...e,
