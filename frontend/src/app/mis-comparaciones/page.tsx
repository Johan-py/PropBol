    ...
    return {
      ...
      propiedades:
        comp.detalle_comparacion?.map((detalle: any) => ({
          ...
          tipoAccion:
            detalle.inmueble.tipo_accion || detalle.inmueble.tipoAccion,
        })) || [],
    };
  };
  ...
  const fetchComparaciones = useCallback(async () => {
    ...
    try {
      ...
      const transformedData = Array.isArray(data)
        ? data.map(transformarComparacion)
        : [];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {Object.entries(grupos).map(([categoria, comparacionesList]) => {
            ...
            return comparacionesList.map((comp) => (
              <section
                key={comp.id}
                className="rounded-3xl border p-6 bg-white shadow-sm flex flex-col"
              >
                ...
                <div
                  className={`grid gap-4 mb-8 grid-cols-1 sm:grid-cols-2 ${comp.propiedades.length > 2 ? "md:grid-cols-3" : ""}`}
                >
