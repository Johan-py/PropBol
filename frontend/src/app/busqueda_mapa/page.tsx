...
import MapaListadoPaginacion, {
  PageSize,
} from "@/components/galeria/MapaListadoPaginacion";
...
function BusquedaMapaContent() {
  ...
  const { getBusquedaModo, cambiarAModoGeneral, clearAllFilters } =
    useSearchFilters();
  ...
  const toggleCapacidad = () => {
    ...
    setActiveSidebarView((prev) =>
      prev === "capacidad" ? "results" : "capacidad",
    );
  };
  ...
  const openEtiquetas = () => {
    ...
    setIsSidebarOpen(true);
    setActiveSidebarView((prev) =>
      prev === "etiquetas" ? "results" : "etiquetas",
    );
  };
  ...
  const [editingPolygonPoints, setEditingPolygonPoints] = useState<
    [number, number][]
  >([]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("abrirPanelUbicacion", handleAbrirUbicacion);
  ...
  const zonasCombinadas = useMemo(() => {
    const zonasUsuarioParaMapa = misZonas
      .map((zonaUsuario) => {
        const coordenadas = extraerCoordenadasDeGeometria(
          zonaUsuario.geometria,
        );
  ...
  useEffect(() => {
    ...
    const zonaVisible = zonasFiltradas.some(
  ...
  const saveDraftZone = useCallback(async () => {
    ...
    try {
      const ring = [...puntosBase, puntosBase[0]].map(([lat, lng]) => [
        lng,
        lat,
      ]);
  ...
  const saveEditedZone = useCallback(async () => {
    if (!editingZoneId || isSavingEditedZone || editingPolygonPoints.length < 3)
      return;
    ...
    try {
      const ring = [...editingPolygonPoints, editingPolygonPoints[0]].map(
        ([lat, lng]) => [lng, lat],
      );
  ...
  const displayedProperties = useMemo(() => {
      ...
      try {
        return properties.filter((p: any) => {
          ...
          return drawnPolygons.some((polyPoints) => {
            ...
            const turfCoords = [...polyPoints, polyPoints[0]].map((p) => [
              p[1],
              p[0],
            ]);
    ...
    if (selectedZoneId !== null) {
      ...
      if (zona && zona.coordenadas && zona.coordenadas.length >= 3) {
        const coords = [...zona.coordenadas, zona.coordenadas[0]].map(
          (c: any) => [c[1], c[0]],
        );
        return properties.filter(
          (p: any) =>
            p.lat != null &&
            booleanPointInPolygon(point([p.lng, p.lat]), polygon([coords])),
        );
      }
    }
    return properties;
                ...
                >
                  ...
                  {viewMode === "grid" ? (
                    <PropertyCard
                      ...
                      precioFormateado={
                        property.precioFormateado || "Consultar precio"
                      }
                      ...
                      precio={
                        property.precio ? Number(property.precio) : undefined
                      }
                      precio_anterior={
      ...
      >
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          <div className="absolute top-3 left-0 right-0 z-[35] flex flex-col items-center gap-2 pointer-events-none">
            ...
            {(drawnPolygons.length > 0 || currentPolygonPoints.length > 0) &&
              !editingZoneId && (
                <div className="flex flex-row gap-2 pointer-events-auto">
                  <button
                    onClick={resetDrawing}
                  ...
                  <div className="mx-4 mb-3 relative shrink-0">
                    ...
                    <div className="ring-2 ring-orange-400 rounded-xl overflow-hidden">
                      <PropertyCard
                        ...
                        precioFormateado={
                          pinnedProperty.precioFormateado || "Consultar precio"
                        }
                        descripcion={
                        ...
                        precio={
                          pinnedProperty.precio
                            ? Number(pinnedProperty.precio)
                            : undefined
                        }
                        precio_anterior={
    ...
    <div className="relative z-10 flex flex-col bg-white dark:bg-stone-950 w-full h-[calc(100dvh-54px)] overflow-hidden">
      <FilterBar
        ...
        onOpenSuperficieFilter={() => {
          ...
          setIsSidebarOpen(true);
          setActiveSidebarView((prev) =>
            prev === "superficie" ? "results" : "superficie",
          );
        }}
        ...
        <aside
          ...
          style={
            isSidebarOpen
              ? {
                  width: isMounted
                    ? activeSidebarView === "results" && !isPriceFilterOpen
                      ? effectiveSidebarWidth
                      : 450
                    : 450,
                }
          ...
          {isPriceFilterOpen ? (
              ...
              <UbicacionEspecificaPanel
                ...
                onApply={(selecciones) => {
                  ...
                  if (selecciones.departamento !== "todos")
                    params.set(
                      "departamentoId",
                      selecciones.departamento.toString(),
                    );
            ...
            <div className="flex flex-col h-full min-h-0 relative bg-stone-50 dark:bg-stone-950">
              ...
              <div className="bg-white dark:bg-stone-900 shrink-0 border-b border-stone-200 dark:border-stone-800 shadow-sm">
                ...
                <div
                  className={`px-4 pb-3 flex flex-col ${isScrolled ? "pt-3 gap-2" : "gap-3"}`}
                >
                  <div
                  ...
                  >
                    <div className="flex min-w-0 justify-between gap-2">
                      <div className="flex min-w-0 flex-col">
                        ...
                        {isRecomendadosActive && !isClusterView && (
                          <p
                            className={`text-gray-500 dark:text-stone-400 ${isScrolled ? "text-[11px]" : "text-xs"}`}
                          >
                            Mostrando resultados personalizados según tu
                            actividad reciente
                          </p>
                        )}
                      </div>
                    ...
                    <div
                      className={`flex w-full flex-col gap-2 ${
                        resultsHeaderSideBySide
                          ? "w-auto max-w-full shrink-0 items-end"
                          : ""
                      }`}
                    >
                      <div
                        ...
                        >
                          ...
                          {viewMode === "grid" ? (
                            <PropertyCard
                              ...
                              descripcion={
                                property.descripcion || property.title
                              }
                              ubicacionTexto={property.ubicacionTexto}
                              categoriaTexto={property.categoriaTexto}
                              accionTexto={property.accionTexto}
              ...
              onMouseDown={(e) => {
                ...
                const onMove = (ev: MouseEvent) => {
                  ...
                  const dynamicMax = Math.min(
                    ...
                    Math.max(
                      SIDEBAR_MIN_WIDTH,
                      window.innerWidth - MAP_MIN_WIDTH,
                    ),
                  );
                  const next = Math.min(
                    dynamicMax,
                    Math.max(SIDEBAR_MIN_WIDTH, startW + dx),
                  );
              ...
              <div className="flex flex-col items-end gap-2 pointer-events-auto">
                ...
                {!drawingError && (
                  <div className="bg-white/95 backdrop-blur-sm p-3 rounded-lg shadow-md border border-stone-200 text-xs text-stone-600 max-w-[220px] text-right">
                    Haz clic en el mapa para marcar los vértices. Cierra la zona
                    tocando el punto inicial.
                  </div>
                )}
              </div>
            )}
      ...
      <CompareFooter
        onOpenModal={() => {
          ...
          if (token && selectedIds.length >= 2) {
            const idsNumericos = selectedIds
              .map((id) => Number(id))
              .filter((id) => !isNaN(id));
            ...
            fetch(`${API_URL}/api/comparaciones`, {
              ...
              console.error("Error al guardar historial de comparación:", err),
            );
          }
        }}
      />
