...
// ─── Fix gray area: invalida el tamaño del mapa cuando el contenedor cambia ──
// window.dispatchEvent(new Event("resize")) desde fuera no siempre es suficiente.
// Este componente vive DENTRO del MapContainer y llama invalidateSize() directamente.
function MapResizer() {
  const map = useMap()

  useEffect(() => {
    const handler = () => {
      // animate:false evita el flash de reposicionamiento
      map.invalidateSize({ animate: false })
    }
    window.addEventListener('resize', handler)
    // Forzar invalidación inmediata al montar (por si el contenedor ya cambió)
    handler()
