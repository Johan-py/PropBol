...
import { PriceTooltip } from "@/components/ClusterSidebar"
...
const PIN_FILL: Record<PropertyMapPin["type"], string> = {
  casa: "#3b82f6", departamento: "#8b5cf6", terreno: "#f59e0b",
  oficina: "#10b981", cuarto: "#ec4899", cementerio: "#64748b", espacios: "#06b6d4"
}
const PIN_HALO: Record<PropertyMapPin["type"], string> = {
  casa: "rgba(59,130,246,0.25)", departamento: "rgba(139,92,246,0.25)", terreno: "rgba(245,158,11,0.25)",
  oficina: "rgba(16,185,129,0.25)", cuarto: "rgba(236,72,153,0.25)", cementerio: "rgba(100,116,139,0.25)", espacios: "rgba(6,182,212,0.25)"
}
const PIN_LABEL: Record<PropertyMapPin["type"], string> = {
  casa: "#2563eb", departamento: "#7c3aed", terreno: "#d97706",
  oficina: "#059669", cuarto: "#db2777", cementerio: "#475569", espacios: "#0891b2"
}
const SELECTED_ICONS: Record<PropertyMapPin["type"], string> = {
  casa: "/house.svg", departamento: "/department.svg", terreno: "/land.svg",
  oficina: "/office.svg", cuarto: "/house.svg", cementerio: "/land.svg", espacios: "/office.svg"
}
...
function createPinIcon(type: PropertyMapPin["type"]): L.DivIcon {
  ...
  const outer = 28; const inner = 20; const half = outer / 2
  return L.divIcon({
    className: "",
    html: `<div style="width:${outer}px;height:${outer}px;display:flex;align-items:center;justify-content:center;"><div style="position:absolute;width:${outer}px;height:${outer}px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background-color:${halo};"></div><div style="position:relative;width:${inner}px;height:${inner}px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background-color:${fill};border:2px solid rgba(255,255,255,0.9);box-shadow:0 1px 4px rgba(0,0,0,0.20);"></div></div>`,
    iconSize: [outer, outer], iconAnchor: [half, outer], popupAnchor: [0, -outer]
  })
}

function ZoomHandler({ onZoom }: { onZoom: () => void }) {
  const map = useMap()
  useEffect(() => {
...
function MapMouseHandler({ onMouseLeave }: { onMouseLeave: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { map.off("mouseout", handleMouseOut) }
  }, [map, onMouseLeave])
  return null
}
...
function createSelectedIcon(type: PropertyMapPin["type"], isHover: boolean = false): L.DivIcon {
  ...
  return L.divIcon({
    className: "",
    html: `<div style="display:flex;align-items:center;justify-content:center;transform:scale(${scale});transition:all 0.15s ease;"><div style="width:36px;height:36px;border-radius:50%;background-color:#ef4444;display:flex;align-items:center;justify-content:center;box-shadow:${shadowIntensity};border:2px solid white;"><img src="${iconPath}" style="width:20px;height:20px;object-fit:contain;display:block;"/></div></div>`,
    iconSize: [36, 36], iconAnchor: [18, 36], popupAnchor: [0, -36]
  })
}

function formatPrice(price: number, currency: "USD" | "BOB"): string {
  return currency === "USD" ? `$${price.toLocaleString("es-BO")} USD` : `Bs ${price.toLocaleString("es-BO")}`
}
...
export default function MapView({
  ...
  useEffect(() => {
    Object.entries(markerRefs.current).forEach(([id, marker]) => {
      if (marker && id !== hoveredPinId && marker.isPopupOpen()) marker.closePopup()
    })
  ...
  const renderMarkers = useMemo(() => {
    return properties.map((property) => {
      ...
      let icon
      if (isSelected) icon = createSelectedIcon(property.type, false)
      else if (isHovered) icon = createSelectedIcon(property.type, true)
      else icon = createPinIcon(property.type)
      return (
        <Marker
          key={property.id}
          ...
          alt={property.id}
          icon={icon}
          ref={(el) => { if (el) markerRefs.current[property.id] = el }}
          eventHandlers={{
            click: () => {
              onSelect?.(property.id)
              setTooltipPinId((prev) => (prev === property.id ? null : property.id))
            },
            mouseover: () => setHoveredPinId(property.id),
            mouseout: () => setHoveredPinId(null)
          }}
        >
          {tooltipPinId === property.id && (
            <div className="relative">
              <PriceTooltip precio={property.price} visible={true} />
            </div>
          )}
          <Popup>
  ...
  }, [properties, selectedId, hoveredPinId, tooltipPinId, onSelect])
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <MapContainer center={center} zoom={zoom} zoomControl={false} touchZoom={true} dragging={true} style={{ height: "100%", width: "100%" }} className="z-0">
        <TileLayer
        ...
        <ZoomHandler onZoom={() => { onSelect?.(null); setTooltipPinId(null) }} />
          <>
            ...
            {polygonPoints.map((pt, index) => (
              <CircleMarker key={index} center={pt} radius={5}
                pathOptions={{ color: index === 0 ? "#ef4444" : "#ea580c", fillColor: "white", fillOpacity: 1 }}
                eventHandlers={{ click: (e) => { L.DomEvent.stopPropagation(e); if (onPointClick) onPointClick(index) } }}
              />
            ))}
          </>
        )}
        {isPolygonClosed && polygonPoints && polygonPoints.length >= 3 && (
          <Polygon positions={polygonPoints} pathOptions={{ color: "#ea580c", fillColor: "#ea580c", fillOpacity: 0.2, weight: 2 }} />
        )}
        {selectedProperty && (
        ...
        <MarkerClusterGroup
          ...
          animate={true} preferCanvas={true} animateAddingMarkers={true} chunkedLoading={true}
          tap={true} tapTolerance={15} showCoverageOnHover={false} zoomToBoundsOnClick={true}
          spiderfyOnMaxZoom={true} spiderfyDistanceMultiplier={2} removeOutsideVisibleBounds={false}
...
function FlyToSelected({ lat, lng, id }: { lat: number; lng: number; id: string }) {
  ...
  useEffect(() => {
    ...
    if (isFar) {
      map.flyTo([lat, lng], targetZoom, { duration: 0.8, easeLinearity: 0.25 })
    } else {
