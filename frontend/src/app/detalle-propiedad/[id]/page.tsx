...
export default function DetallePropiedadPage() {
  ...
  const { isFavorite, isLoadingStatus, isSubmitting, toggleFavorite } =
    useFavorite({
  ...
  const esOferta =
    detalleAny.precio_anterior &&
    detalle.precio &&
    detalle.precio < detalleAny.precio_anterior;
  const porcentajeDescuento = esOferta
    ? Math.round(
        ((detalleAny.precio_anterior - detalle.precio) /
          detalleAny.precio_anterior) *
          100,
      )
    : 0;
      ...
      <div className="mx-auto w-full max-w-[1120px] px-4 py-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_300px] lg:items-start">
          <div className="space-y-5">
            ...
            <UbicacionPropiedad
              mapa={detalle.mapa}
              puntosDeInteres={detalle.puntosDeInteres}
            />
          </div>
