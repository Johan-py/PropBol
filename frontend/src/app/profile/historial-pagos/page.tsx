...
const metodoPagoBadge: Record<string, { label: string; cls: string }> = {
  ...
  QR_BANCARIO_MENSUAL: {
    label: "QR Bancario (mens.)",
    cls: "bg-amber-50 text-amber-700 border-amber-100",
  },
  QR_BANCARIO_ANUAL: {
    label: "QR Bancario (anual)",
    cls: "bg-amber-50 text-amber-700 border-amber-100",
  },
  USDT_TRC20: {
    label: "USDT TRC20",
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 rounded-2xl bg-stone-200 animate-pulse"
              />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="space-y-3">
            {transacciones.map((t) => {
              ...
              const metodoBadge = metodoPagoBadge[t.metodo_pago] ?? {
                label: t.metodo_pago,
                cls: "bg-stone-50 text-stone-600 border-stone-200",
              };
                  ...
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-bold font-montserrat text-stone-900">
                          {t.plan}
                        </span>
                        <span
                      ...
                      <span
                        className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${metodoBadge.cls}`}
                      >
                        {metodoBadge.label}
                      </span>
                    </div>
                    <div className="text-right">
                      ...
                      {t.monto_descuento > 0 && (
                        <p className="text-xs text-green-600">
                          -Bs. {t.monto_descuento.toFixed(2)} descuento
                        </p>
                      )}
                    </div>
                  </div>
