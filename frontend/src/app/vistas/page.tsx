  ...
  return (
    <div className="group bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden relative hover:shadow-md transition-all">
      ...
      <div className="p-4 relative">
        ...
        <p className="text-black text-[11px] mt-1 font-medium italic">
          {prop.location}
        </p>
...
export default function VistasRecientesPage() {
  ...
  const fetchHistorial = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/perfil/historial/vistas`,
        {
  ...
  const renderMonth = (date: Date, offset: number) => {
    ...
    const monthName = target.toLocaleString("es-ES", {
      month: "long",
      year: "numeric",
    });
      ...
      <div className="flex-1 min-w-[200px]">
        ...
        <div className="grid grid-cols-7 text-center text-[10px]">
          {["do", "lu", "ma", "mi", "ju", "vi", "sá"].map((d) => (
            <div
              key={d}
              className="text-gray-400 font-semibold mb-2 capitalize"
            >
              {d}
            </div>
          ))}
          ...
          {[...Array(daysInMonth)].map((_, i) => {
            ...
            const inRange =
              rangeStart && rangeEnd && dObj > rangeStart && dObj < rangeEnd;
            ...
            return (
              ...
              >
                <span
                  className={`z-10 w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold transition-all
  ...
  const handleClearHistory = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/perfil/historial/limpiar`,
        {
          method: "PATCH",
          headers: {
