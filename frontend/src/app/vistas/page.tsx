...
export default function VistasRecientesPage() {
  ...
  const handleDateFilter = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = properties.filter(
      (p) => new Date(p.viewedDate).toISOString().split('T')[0] === selectedDate
    )
  ...
  if (loading)
    return <div className="p-20 text-center font-bold text-black">Conectando con PropBol...</div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredProperties.length > 0 ? (
            filteredProperties.map((prop: any) => <PropertyCard key={prop.id} prop={prop} />)
          ) : (
