...
export default function PaginaResumenCompra() {
  ...
  const manejarContinuar = () => {
    if (metodoSeleccionado && transaccion) {
      // Guardar en localStorage para que la página QR lo recupere
      localStorage.setItem('currentPayment', JSON.stringify({
        id: transaccion.id,
        monto: transaccion.total,
        referencia: transaccion.referencia || transaccion.id,
        estado: 'pendiente',
        fechaExpiracion: transaccion.fechaExpiracion || new Date(Date.now() + 30 * 60 * 1000).toISOString(),
        qrContent: transaccion.plan_suscripcion?.imagen_gr_url || '/qrs/estandar.png',
      }));
  ...
  const aplicarCupon = async () => {
    ...
    try {
      ...
      setMensajeCupon({ texto: err.message, error: true });
    } finally {
      setAplicandoCupon(false);
    }
  };

  if (cargando) return <div className="text-center py-10 text-gray-600">Cargando...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          <div className="border border-gray-200 rounded-lg p-6 mb-6 shadow-md">
            ...
            <p className="text-gray-600">
              {plan.nro_publicaciones_plan} publicaciones activas · Vigencia {plan.duración_plan_días} días · Galería {plan.fotos_galeria} fotos
            </p>
          </div>
          ...
          <div className="border border-gray-200 rounded-lg p-6 bg-white shadow-md">
            ...
            >
              Continuar con {metodoSeleccionado ? nombreMetodo[metodoSeleccionado] : 'un método'}
            </button>
        ...
        <div>
          ...
          <div className="mt-6 p-4 border border-gray-200 rounded-lg bg-gray-50">
            ...
            {mensajeCupon && (
              <p className={`text-xs mt-2 ${mensajeCupon.error ? 'text-red-600' : 'text-green-600'}`}>
                {mensajeCupon.texto}
              </p>
            )}
          </div>
...
>>>>>>> ad042467 (Feat: Se agrega funcionalidad de aplicacion de descuento por cupón)
  );
}